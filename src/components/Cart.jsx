...
export default function Cart() {
    ...
    return <Modal className="cart" open={userProgressCTX.progress === 'cart'}>
        <h2>Your Cart</h2>
        <ul>
            {cartCTX.items.map(item => <CartItem key={item.id} name={item.name} price={item.price} quantity={item.quantity} onDecrease={() => cartCTX.removeItem(item.id)} onIncrease={() => cartCTX.addItem(item)} />)}
        </ul>
